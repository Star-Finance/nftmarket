...
for (let i = 1; i <= 5; i++) {
    _allNFTS.push({
        attachment: require(`../img/boom_${i}.png`),
        token_id: 8000 + i
    })
}
const _tokenIdToImgMap = {}
export default forwardRef((props: IProps, ref) => {
    const { account, onPlayComplete, onOpenWinModal } = props
    ...
    }, [account, nft1155Contract, dodoContract, gateContract])
    ...
    const getCards = async (): Promise<void> => {
        ...
        //         console.log('WhiteCardTokenId', i)
        ...
        // if (dodo_balance > 0) {
        //     const dodo_token_id = await dodoContract.tokenOfOwnerByIndex(account, 1)
        //     setWhiteCardTokenId(+dodo_token_id)
        //     return
        // }
        const gate_balance = await gateContract.balanceOf(account)
        if (gate_balance > 0) {
            const gate_token_id = await gateContract.tokenOfOwnerByIndex(account, 1)
    ...
    const play = async (): Promise<void> => {
        ...
        try {
            ...
            console.log('BoomContract', BoomContract)
            const BoomContractRes = await BoomContract.attach(config.boomContract)
            console.log('BoomContractRes', BoomContractRes)
    ...
    const openWin = async (args: any): Promise<void> => {
        onOpenWinModal(-1)
                ...
                >
                    ...
                    <CSSTransition in={!showRecommend} timeout={500} classNames="fade-scale" unmountOnExit>
                        ...
                        >
                            ...
                            >
                                Recommended combination{whiteCardTokenId}
                            </Button>
